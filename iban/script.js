...
function addNewClient() {
  ...
  const lastIban = clientAccounts[clientAccounts.length - 1].iban.replace(/\s/g, '');
  const nationalPart = lastIban.substring(2);
  const currentNum = nationalPart.substring(start - 1, end);
  const nextNum = (parseInt(currentNum, 10) + 1).toString().padStart(editableLength, '0');

  if (parseInt(nextNum) >= maxValue) {
    alert("تم استهلاك جميع الأرقام الممكنة في هذا النطاق.");
    return;
  }

  const national = prefix + nextNum + suffix;
  const iban = "SA" + national;
  const formattedIban = iban.match(/.{1,4}/g).join(" ");
  const autoId = generateId(clientAccounts.length);
